...
LOW_POTASSIUM_INGREDIENTS: List[Dict[str, Any]] = [
...
HIGH_POTASSIUM_INGREDIENTS: List[Dict[str, Any]] = [
...
LOW_POTASSIUM_PROCESSED: List[Dict[str, Any]] = [
...
HIGH_POTASSIUM_PROCESSED: List[Dict[str, Any]] = [
...
LOW_POTASSIUM_DISHES: List[Dict[str, Any]] = [
...
HIGH_POTASSIUM_DISHES: List[Dict[str, Any]] = [
...
LOW_PHOSPHORUS_INGREDIENTS: List[Dict[str, Any]] = [
...
HIGH_PHOSPHORUS_INGREDIENTS: List[Dict[str, Any]] = [
...
LOW_PHOSPHORUS_PROCESSED: List[Dict[str, Any]] = [
...
HIGH_PHOSPHORUS_PROCESSED: List[Dict[str, Any]] = [
...
LOW_PHOSPHORUS_DISHES: List[Dict[str, Any]] = [
...
HIGH_PHOSPHORUS_DISHES: List[Dict[str, Any]] = [
...
def get_all_potassium_data() -> Dict[str, List[Dict[str, Any]]]:
    """모든 칼륨 관련 데이터 반환"""
    return {
        "low_ingredients": LOW_POTASSIUM_INGREDIENTS,
        "high_ingredients": HIGH_POTASSIUM_INGREDIENTS,
        "low_processed": LOW_POTASSIUM_PROCESSED,
        "high_processed": HIGH_POTASSIUM_PROCESSED,
        "low_dishes": LOW_POTASSIUM_DISHES,
        "high_dishes": HIGH_POTASSIUM_DISHES,
    }

def get_all_phosphorus_data() -> Dict[str, List[Dict[str, Any]]]:
    """모든 인 관련 데이터 반환"""
    return {
        "low_ingredients": LOW_PHOSPHORUS_INGREDIENTS,
        "high_ingredients": HIGH_PHOSPHORUS_INGREDIENTS,
        "low_processed": LOW_PHOSPHORUS_PROCESSED,
        "high_processed": HIGH_PHOSPHORUS_PROCESSED,
        "low_dishes": LOW_PHOSPHORUS_DISHES,
        "high_dishes": HIGH_PHOSPHORUS_DISHES,
    }
...
def search_food_by_name(name: str) -> List[Dict[str, Any]]:
    ...
    return [food for food in all_foods if name.lower() in food["name"].lower() or name.lower() in food["name_en"].lower()]

def get_food_risk_level(food_name: str, nutrient: str = "potassium") -> str:
    """음식의 위험 수준 반환"""
    if nutrient == "potassium":
        low_foods = LOW_POTASSIUM_INGREDIENTS + LOW_POTASSIUM_PROCESSED + LOW_POTASSIUM_DISHES
        high_foods = HIGH_POTASSIUM_INGREDIENTS + HIGH_POTASSIUM_PROCESSED + HIGH_POTASSIUM_DISHES
    else:
        low_foods = LOW_PHOSPHORUS_INGREDIENTS + LOW_PHOSPHORUS_PROCESSED + LOW_PHOSPHORUS_DISHES
        high_foods = HIGH_PHOSPHORUS_INGREDIENTS + HIGH_PHOSPHORUS_PROCESSED + HIGH_PHOSPHORUS_DISHES

    for food in low_foods:
        if food_name.lower() in food["name"].lower():
            return "low"
    for food in high_foods:
        if food_name.lower() in food["name"].lower():
            return "high"
    return "unknown"
