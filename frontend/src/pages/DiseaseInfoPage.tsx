  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        if (profile?.diseaseInfo) {
          const d = profile.diseaseInfo;
          setDiagnosisType(d.diagnosisType || 'UNKNOWN');
          setCkdStage(d.ckdStage || 'NONE');
          setDialysisType(d.dialysisType || 'NONE');
          setBaseConditions(d.baseConditions || []);
          setOtherConditionMemo(d.otherConditionMemo || '');
        }
      } catch (error) {
    ...
    return (
      <div className="flex flex-col h-full bg-white">
        <div className="lg:hidden">
          <MobileHeader title="질환정보" showProfile={false} showMenu={false} onBack={() => navigate('/mypage')} />
        </div>
